...
const MobileMenu = ({ setIsMenuOpen, menuRef }) => {
    return (
        <>
            ...
            <div
                ref={menuRef}
                className={` fixed top-0 translate-x-[110%] h-screen w-screen bg-gray-950/80 backdrop-blur-sm shadow-lg z-50 `}
                ...
                <div className="px-10 pt-[50px] h-full">
                    ...
                    <div className=' flex flex-col items-start gap-10 pt-[40px] text-xl font-light underline-offset-4 text-[#c1c0c0]'>
                        <Link onClick={() => {
                            ...
                            Home
                            <span className="absolute nav-menu-underline -bottom-1 left-0 w-full h-[2px] bg-indigo-400 group-hover:w-full transition-all duration-300"></span>
                        </Link>
                        <Link onClick={() => {
                            document.body.style.overflowY = "auto"
                            setIsMenuOpen(false)
